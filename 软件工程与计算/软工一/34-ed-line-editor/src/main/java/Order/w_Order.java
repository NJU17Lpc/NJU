...
public class w_Order implements Order{
	ED_Buffer edBuffer = null;
	String order="";
	
	public w_Order(String order, ED_Buffer edBuffer) {
		this.order = order;
		...
		if(processOrder.equals("w")) {
			...
			if(edBuffer.checkFilePath.equals("")) {
				...
				if(startPosition<0||endPosition>=edBuffer.textContent.size()||startPosition>endPosition){
					...
					edBuffer.flushTextContentData(tempList,edBuffer.checkFilePath);
				}
			}
			...
			if(startPosition<0||endPosition>=edBuffer.textContent.size()||startPosition>endPosition){
				...
				edBuffer.flushTextContentData(tempList, processOrder.substring(2));
			}
